...
@ToolParser.register("llama-3.1")
class Llama31ToolParser(ToolParser):
    """
    Tool parser for Llama-3.1 models using the native HuggingFace chat template.

    Llama 3.1 tool calls are delimited by ``<|python_tag|>`` (start) and ``<|eom_id|>`` (end),
    with JSON content ``{"name": "...", "parameters": {...}}``.

    Use this parser together with ``format: llama-3.1`` in the config so that
    ``apply_chat_template(tools=...)`` activates the native Llama template and
    the correct delimiters are expected during extraction.
    """
    ...
    def __init__(self, tokenizer) -> None:
        ...
        self.tool_call_regex = regex.compile(
            r"<\|python_tag\|>(.*?)(?:<\|eom_id\|>|$)", regex.DOTALL
        )

    @rollout_trace_op
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        loop = get_event_loop()
        # Keep special tokens so we can detect <|python_tag|> and <|eom_id|>
        ...
        if self.tool_call_start_token not in text:
            # Decode without special tokens for clean content
            clean = await loop.run_in_executor(None, self.tokenizer.decode, responses_ids)
            return clean, []
        ...
        content = self.tool_call_regex.sub("", text).replace(self.tool_call_start_token, "").strip()
        return content, function_calls
